...
public class Town {
	...
	public Town(int id, String name, Kingdom kingdom, String townBoard, int defaultPlotPrice,
		...
		this.setAllFields(id, name, kingdom, townBoard, defaultPlotPrice, spawnLocation, isPublic, isCapital,
	...
	public int getMaxLandSize() {
		return (residents.size() * KingdomsMain.plugin.config.getInt(String.format(
		        KingdomsConfigHelper.TOWN_PLOTS_PER_RESIDENT, this.getSpawnLocation().getWorld().getName())))
		        + numBonusPlots;
	}
	...
	public TownWarp getWarp(String name) {
		if (warps.get(name.toLowerCase()) != null) {
			return KingdomsManager.getTownWarp(warps.get(name.toLowerCase()));
		}
		return null;
	}
	...
	public boolean hasStaff(String resident) {
		if (getKingdom() != null && getKingdom().getKing().getName().equalsIgnoreCase(resident)) {
			return true;
		}
		if (getMayor().getName().equalsIgnoreCase(resident)) {
			return true;
		}
	...
	public void unclaim(KingdomsChunk chunk) {
		...
		for (int i = 0; i < land.size(); i++) {
			int index = land.get(i);
			if (index == chunk.getId()) {
				chunkIndex = i;
				break;
			}
		}
	...
	public Location getClosestLocation(Location location) {
		Location closestLocation = new Location(KingdomsManager.getKingdomsChunk(land.get(0)).getWorld(),
		        KingdomsManager.getKingdomsChunk(land.get(0)).getX() * 16, location.getY(), KingdomsManager
		                .getKingdomsChunk(land.get(0)).getZ() * 16);
		for (int i = 1; i < land.size(); i++) {
			...
			if (chunk != null) {
				if ((location.getChunk().getX() > chunk.getX() && chunk.getX() > closestLocation.getChunk().getX()
				        && location.getChunk().getZ() > chunk.getZ() && chunk.getZ() > closestLocation.getChunk()
				        .getZ())
				        || (location.getChunk().getX() > chunk.getX()
				                && chunk.getX() > closestLocation.getChunk().getX()
				                && location.getChunk().getZ() < chunk.getZ() && chunk.getZ() < closestLocation
				                .getChunk().getZ())
				        || (location.getChunk().getX() < chunk.getX()
				                && chunk.getX() < closestLocation.getChunk().getX()
				                && location.getChunk().getZ() > chunk.getZ() && chunk.getZ() > closestLocation
				        ...
				        || (location.getChunk().getX() < chunk.getX()
				                ...
				                && location.getChunk().getZ() < chunk.getZ() && chunk.getZ() < closestLocation
				                .getChunk().getZ())) {
					closestLocation = new Location(chunk.getWorld(), chunk.getX() * 16, location.getY(),
					        chunk.getZ() * 16);
				}
			}
		}
	...
	public List<String> showInfo(boolean onlineList) {
		...
		out.add("&" + outputColor[0] + "Size: &" + outputColor[1] + this.getLandSize() + "    &" + outputColor[0]
		        + "Max Size: &" + outputColor[1] + this.getMaxLandSize());
		if (this.getKingdom() != null) {
			out.add("&" + outputColor[0] + "Kingdom: &" + outputColor[1] + this.getKingdom().getName());
		}
		if (this.getMayor() != null) {
			out.add("&" + outputColor[0] + this.getMayor().getTownFriendlyTitle() + ": &" + outputColor[1]
		...
		if (hasUpdated) {
			DeityAPI.getAPI()
			        ...
			        .write("UPDATE "
			                ...
			                (kingdom != null ? kingdom.getId() : -1), townBoard, defaultPlotPrice,
			                spawnLocation.getId(), (isPublic() ? 1 : 0), (isCapital() ? 1 : 0), creationDate,
			                numBonusPlots, this.permissions.get(DeityChunkPermissionTypes.EDIT).ordinal(),
	...
	public void remove() {
		...
		DeityAPI.getAPI().getDataAPI().getMySQL()
		        .write("DELETE FROM " + KingdomsMain.getTownTableName() + " WHERE id = ?;", id);
	}
}
